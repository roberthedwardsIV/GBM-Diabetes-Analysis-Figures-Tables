...
df <- within(df, {
  ...
  # 💥 FIX: SIMPLIFY AGE.CLASS LABELS (NO SPACES/PERIODS) TO PREVENT STRING PARSING ERRORS
...
display_df_d <- res_d %>%
  mutate(
    ...
    # NEW: Clean up the extracted level names to match the display labels (using the non-ref levels)
    Level = case_when(
  ...
  # Filter to KEEP ONLY the desired factor
...
table_2d <- ggplot(plot_df_d, aes(y = Level, x = estimate), title = "Multivariate Analysis of Prognostic Factors (Combined Groups)") +
  ...
  scale_x_log10(limits = c(0.01, 70), n.breaks = 4) +
...
combined <- (figure_threeD$plot + figure_threeE$plot + figure_threeF$plot + figure_threeG$plot) +
